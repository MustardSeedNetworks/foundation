...
| `github.com/MustardSeedNetworks/foundation/pkg/supervise` | Named goroutines under one supervisor: a panic recovered into an error, a per-worker `Fatal` / `RestartN(n)` policy, and shutdown in reverse registration order bounded by the caller's deadline. One error line for the operator; the stack only at debug level. |
